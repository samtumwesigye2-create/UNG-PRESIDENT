...
def test_nsc_record_creation(tmp_path):
 with setup_client(tmp_path) as client:
  login(client);r=client.post('/nsc/briefing',data={'title':'Morning brief','summary':'Executive briefing','owner':'NSC Secretariat','classification':'secret'},follow_redirects=False);assert r.status_code==303
  page=client.get('/nsc/briefing');assert 'Morning brief' in page.text and 'secret' in page.text
